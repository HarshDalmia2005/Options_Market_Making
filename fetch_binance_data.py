...
def main():
    ...
    try:
        ...
        print(f"Current BTC Spot: ${S_spot:.2f}")
    ...
    # Filter for active CALL options on BTC
    active_calls = [s for s in symbols if s['underlying'] == 'BTCUSDT' and s['side'] == 'CALL' and s['status'] == 'TRADING']
    ...
    for s in atm_calls:
        ...
        # Calculate original z_i or use from table (z_i = 500000 / price)
        # We'll recompute a z_i for the real trades so risk isn't crazy
        ...
        for i, t in enumerate(trades):
            ...
            # z_i in the model is large (e.g. 500,000 / price). We just use the model's z_i to match the HJB grid scale!
            # Otherwise our Vpi will barely move.
    ...
    df_out.to_csv("data/binance_historical_data.csv", index=False)
    
    print(f"\nSuccess! Saved {len(df_out)} real Binance trades to data/binance_historical_data.csv")
